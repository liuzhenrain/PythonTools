...
def _read_excel_data(workbook, filename, sheetname, ismain, excel_data_dic={}):
    ...
    try:
        ...
        if ismain == True:
            ...
            LogCtrl.log(u"文件名%s和主表名%s字不一致，请修改EXCEL文件" % (filename, sheetname))
            return excel_data_dic
        else:
            # print "Excel文件:%s中没有%s这个表" % (filename, sheetname)
            LogCtrl.log(u"Excel文件:%s中没有%s这个表" % (filename, sheetname))
            return excel_data_dic
    ...
    for rowIndex in range(dataStartRow, rowcount):
        ...
        if dataKey == "":
            LogCtrl.log(u"%s表第%s行Key数据为空，直接跳过." % (sheetname, rowIndex + 1))
            for colIndex in real_data_col:
                # exportType = get_cell_data(worksheet, exportTypeRow, colIndex)
                # if exportType.lower() == "s":
                #     continue
                # else:
                value = u"%s表第%s行Key数据为空,其余值全部填写同样的数据" % (sheetname, rowIndex + 1)
...
def readexcel(filename):
    LogCtrl.log(u".......当前EXCEL : %s ......" % filename)
